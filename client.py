...
def dosya_gonder(dosya_yolu):
    ...
    # CSV Log dosyasını açıyoruz
    log_dosya_adi = f"client_log_{int(baslangic)}.csv"
    with open(log_dosya_adi, mode='w', newline='', encoding='utf-8') as csv_file:
        log_writer = csv.writer(csv_file)
        log_writer.writerow(['timestamp', 'event_type', 'seq_no', 'attempt_no', 'rtt_seconds'])
        ...
        for i in range(toplam_paket):
            ...
            for deneme in range(1, MAX_DENEME + 1):
                ...
                # Gönderim olayını logla
                log_writer.writerow([send_time, 'SEND', i, deneme, ''])
                ...
                try:
                    ...
                    if tip == 2 and ack_seq == i:
                        ...
                        print(f"  ACK alındı: seq={i} (RTT: {rtt:.4f}s)")
                        
                        # Başarılı ACK olayını logla
                        log_writer.writerow([ack_time, 'ACK', i, deneme, rtt])
                    ...
                    # Zaman aşımı olayını logla
                    log_writer.writerow([timeout_time, 'TIMEOUT', i, deneme, ''])
